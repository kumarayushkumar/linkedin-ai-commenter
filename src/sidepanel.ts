...
class SidePanelUI {
  ...
  constructor() {
    ...
    this.elements = {
      ...
      // Settings elements
      promptInput: document.getElementById('customPrompt') as HTMLTextAreaElement || undefined,
      status: document.getElementById('status') || undefined,
      activeToggle: document.getElementById('activeToggle') as HTMLInputElement || undefined,
  ...
  private initTabSwitching(): void {
    if (!this.elements.settingsTab || !this.elements.responseTab || 
        !this.elements.settingsContent || !this.elements.responseContent) return;
    ...
    this.elements.settingsTab.addEventListener('click', () => {
      ...
      if (this.elements.settingsContent) this.elements.settingsContent.style.display = '';
      if (this.elements.responseContent) this.elements.responseContent.style.display = 'none';
    });
    ...
    this.elements.responseTab.addEventListener('click', () => {
      ...
      if (this.elements.responseContent) this.elements.responseContent.style.display = '';
      if (this.elements.settingsContent) this.elements.settingsContent.style.display = 'none';
      this.fetchVariants();
    });
  }
        ...
        navigator.clipboard.writeText(text)
          .then(() => {
            if (this.elements.responseStatus) {
              this.elements.responseStatus.textContent = 'Comment copied to clipboard!';
              setTimeout(() => {
          ...
          .catch(err => {
            ...
            if (this.elements.responseStatus) {
              this.elements.responseStatus.textContent = 'Failed to copy comment';
              setTimeout(() => {
                if (this.elements.responseStatus) this.elements.responseStatus.textContent = '';
              }, 3000);
            }
  ...
  private async fetchVariants(): Promise<void> {
    ...
    const variants = this.elements.gptResponses.querySelectorAll<HTMLElement>('.response-variant');
    ...
    try {
      ...
      if (!postText) {      variants.forEach((variant: HTMLElement) => {
      ...
      const comments = await openAIService.generateComment(postText, customPrompt);
      ...
      if (this.elements.responseStatus) {
        this.elements.responseStatus.textContent = error.userMessage || 'Failed to generate comments';
      }
  ...
  private initSettingsHandlers(): void {
    ...
    const resetBtn = document.getElementById('resetPrompt') as HTMLButtonElement;
    
    if (!saveBtn || !resetBtn || !this.elements.promptInput || !this.elements.activeToggle || !this.elements.status) return;
    ...
    this.elements.activeToggle.addEventListener('change', async () => {
      if (this.elements.activeToggle) {
        await StorageService.set({ [STORAGE_KEYS.EXTENSION_ACTIVE]: this.elements.activeToggle.checked });
      }
    });
  }
  ...
  private async loadSettings(): Promise<void> {
    ...
    if (this.elements.activeToggle) {
      this.elements.activeToggle.checked = result[STORAGE_KEYS.EXTENSION_ACTIVE] !== false;
    }
  }
  ...
  private switchToResponseTab(): void {
    if (!this.elements.responseTab || !this.elements.settingsTab || 
        !this.elements.responseContent || !this.elements.settingsContent) return;
