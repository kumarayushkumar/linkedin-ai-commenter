// Fixed NodeJS.Timeout error by using global setTimeout type
...
export function extractPostText(postElement: HTMLElement | null): string {
  ...
  postText = postText.replace(/(\u2026more|\.\.\.more|See more)/gi, '').trim();
 ...
 */
export function debounce<T extends (...args: any[]) => void>(func: T, wait = 300): (...args: Parameters<T>) => void {
 ...
 */
export function throttle<T extends (...args: any[]) => void>(func: T, limit = 300): (...args: Parameters<T>) => void {
  let lastCall = 0;
  return function(...args: Parameters<T>) {
    const now = Date.now();
...
export function formatErrorMessage(error: Error): string {
  if (error.message.includes('API key')) {
    return 'API key not configured. Please update your settings.';
  }
